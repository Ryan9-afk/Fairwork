...
Fairwork Pulse uses open-source software including Next.js, React, TypeScript, Tailwind CSS, Lucide icons, Supabase JavaScript/SSR clients, and supporting packages listed in `package.json`. Their respective licences and repository notices apply.

The interface uses the self-hosted Geist font as a fallback on devices without Apple system fonts. The CEPCJ logo supplied by the team may be used only to identify the hackathon organiser and does not imply product endorsement.
